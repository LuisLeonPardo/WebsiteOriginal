...
const registerUser = async (req, res, next) => {
  try {
    ...
    return res
      .status(500)
      .send({ info: "Ya existe una cuenta con ese gmail", success: false });
    // next(err);
  }
};
...
const registerAdmin = async (req, res, next) => {
  try {
    ...
    return res
      .status(500)
      .send({ info: "Ya existe una cuenta con esa wallet", success: false });
    // next(err);
  }
};
...
const login = async (req, res, next) => {
  ...
  if (!email || !password)
    return next(
      new ErrorResponse("Por favor provea un email y contraseña", 400, false)
    );
  ...
  try {
    ...
    if (!user)
      return res
        .status(401)
        .send({ info: "Credenciales Invalidas", success: false }); // return next(new ErrorResponse('Credenciales Invalidas', 401, false))
    if (!user.estado)
      return res
        .status(401)
        .send({ info: "Usuario baneado permanentemente", success: false });
    if (new Date().toString().slice(4, 24) < user.timeBanned)
      return res.send({
        info: `Usuario baneado hasta ${user.timeBanned}`,
        success: false,
        user,
      });
    ...
    return res
      .status(401)
      .send({ info: "Credenciales Invalidas", success: false });
...
const forgotPassword = async (req, res, next) => {
  ...
  try {
    const user = await User.findOne({ email });
    if (!email)
      return next(new ErrorResponse("Por favor provea un email", 400));
    ...
    try {
      ...
      res.send({
        info: "Se ha enviado un email con instrucciones para resetear la contraseña",
        success: true,
        token: resetToken,
      });
    } catch (err) {
...
const googleLogin = async (req, res) => {
  try {
    ...
    if (!email_verified)
      return res
        .status(400)
        .json({ info: "Email verification failed.", success: false });
    ...
    if (user) {
      ...
      if (!match)
        return res
          .status(400)
          .json({ info: "Password is incorrect.", success: false });
      if (!user.estado)
        return res
          .status(401)
          .send({ info: "Usuario baneado permanentemente", success: false });
      ...
      const newUser = new User({
        username: verify.payload.sub[3] + name,
        name,
        email,
        password,
        Image: picture,
